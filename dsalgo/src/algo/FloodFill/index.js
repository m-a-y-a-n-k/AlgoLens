...
class CanvasImage extends React.Component {
    ...
    fillPixel(fillColor,xPos,yPos,firstRgb,image){
        ...
        image
            .getContext('2d')
            .putImageData( imageData, xPos,yPos );

        // Flood fill recursion
        setTimeout(()=>{this.fillPixel(fillColor,xPos + 1,yPos,firstRgb,image)},20);
        setTimeout(()=>{this.fillPixel(fillColor,xPos + 1,yPos - 1,firstRgb,image)},20);
        setTimeout(()=>{this.fillPixel(fillColor,xPos + 1,yPos + 1,firstRgb,image)},20);
        setTimeout(()=>{this.fillPixel(fillColor,xPos,yPos - 1,firstRgb,image)},20);
        setTimeout(()=>{this.fillPixel(fillColor,xPos,yPos + 1,firstRgb,image)},20);
        setTimeout(()=>{this.fillPixel(fillColor,xPos - 1,yPos,firstRgb,image)},20);
        setTimeout(()=>{this.fillPixel(fillColor,xPos - 1,yPos + 1,firstRgb,image)},20);
        setTimeout(()=>{this.fillPixel(fillColor,xPos - 1,yPos - 1,firstRgb,image)},20);
...
export default class FloodFill extends React.Component {
    ...
    getCustomImage(event) {
        ...
        if(file.type.indexOf('image') === -1){
            self.setState({ hasImage: false, alert: 'Please upload an image' }, () => {
                setTimeout(() => {
